...
function Switcher({ firstImage, secondeImage }: SwitcherProps) {
  return (
    <label className=" relative flex items-center w-32 h-12  rounded-full shadow-[0_5px_5px_0px_rgba(0,0,0,0.2)]">
      ...
      <span className="absolute inset-0 cursor-pointer bg-[#94E3C2] rounded-full" />
      ...
      <span className="absolute shadow-lg m-3 w-14 h-9 bg-white bg-opacity-20 rounded-full transition duration-300 transform peer-checked:translate-x-[48px]" />
    </label>
  );
}
